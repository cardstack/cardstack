...
module.exports = declareInjections(
  ...
  class Schema {
    ...
    equalTo(otherSchema) {
      return otherSchema && isEqual(new Set(this._originalModels), new Set(otherSchema._originalModels));
    }
    ...
    abstractRealms() {
      if (!this._abstractRealms) {
        this._abstractRealms = uniqWith(
          this._allGrants.map(({ who }) => {
    ...
    async userRealms(documentContext) {
      ...
      let groups = (await contentType.groups(documentContext)).map(group =>
        Session.encodeBaseRealm('groups', group.id)
      );
      ...
      if (groups.length === 0) {
        ...
        let hits = this.abstractRealms().filter(abstractRealm =>
          abstractRealm.statics.every(realm => groups.includes(realm))
        );
        ...
        return hits.map(abstractRealm => {
          if (abstractRealm.dynamicSlots > 0) {
            return uniq([...abstractRealm.statics, ownBaseRealm])
              .sort()
              .join('/');
          } else {
            return uniq(abstractRealm.statics)
              .sort()
              .join('/');
          }
        });
