...
module.exports = class Group {
  ...
  async test(documentContext) {
    ...
    return (
      await Promise.all(
        [...this._fieldFilters.entries()].map(async ([fieldName, allowedValues]) => {
...
const validatedFieldFilters = function(allFields, searchQuery, model) {
  ...
  for (let [field, values] of Object.entries(searchQuery.filter)) {
    ...
    if (!allFields.has(field)) {
      throw new Error(
        `group ${model.id}'s search query is targeting unknown field ${field}: ${JSON.stringify(searchQuery, null, 2)}`
      );
    }
    if (!values.exact) {
      throw new Error(
        `group ${model.id}'s search query must use an exact filter for field ${field}: ${JSON.stringify(
          searchQuery,
          null,
          2
        )}`
...
const validatedTypes = function(searchQuery, model) {
  ...
  if (!types) {
    throw new Error(
      `group ${model.id} search-query is required to filter by exact types: ${JSON.stringify(searchQuery, null, 2)}`
    );
  }
