...
module.exports = declareInjections(
  ...
  class Writers {
    ...
    async handleCardOperations(session, card, isDeletion) {
      ...
      let deletedModels = [];
      let changedModels = [];
      let ignoredModels = (internalCard.included || [])
        .filter(i => getCardId(i.id) !== id)
      ...
      let beforeFinalize = async currentSchema => {
        ...
        let localSchema = await currentSchema.applyChanges(
          deletedModels.map(document => ({ type: document.type, id: document.id, document }))
        );
        ...
        for (let resource of addedModels) {
          ...
          let includedResource = (internalCard.included || []).find(
      ...
      let afterFinalize = async (originalDocumentSchema, newDocumentSchema) => {
        ...
        let updatedSchema = await this._deleteInternalCardResources(
          session,
          originalDocumentSchema || newDocumentSchema,
          deletedModels
        );
    ...
    async handleCreate(isBinary, session, type, documentOrStream, schema, indexOnly) {
      ...
      if (type === 'cards') {
        ...
        ({ context, schema, internalCard: documentOrStream, beforeFinalize } = await this.handleCardOperations(
          session,
          documentOrStream
        ));
        ({ type, id } = documentOrStream.data);
        performanceLog.debug(
          `create ${type}/${documentOrStream.data.id} time to complete handleCardOperations: ${Date.now() -
            handleCardOperationsStart}ms`
        );
      }

      let pending;
      let { writer, sourceId } = await this._getSchemaDetailsForType(schema, type, id, documentOrStream);
      if (isInternalCard(type, documentOrStream.data && documentOrStream.data.id) && !writer.hasCardSupport) {
        let source = schema.getDataSource(sourceId);
        throw new Error(
          `The configured writer for cards documents, 'data-sources/${sourceId}' (source type '${source.sourceType}'), does not have card support`
        );
      }
      if (isBinary) {
        let opts = await writer.prepareBinaryCreate(session, type, documentOrStream);
        ...
        let document =
          isInternalCard(type, documentOrStream.data && documentOrStream.data.id) && writer.hasCardSupport
            ? this._cleanupCardData(schema, documentOrStream)
            : this._cleanupBodyData(schema, documentOrStream.data);
        if (!indexOnly) {
          let opts = await writer.prepareCreate(session, type, document, isSchema);
          let { originalDocument, finalDocument, finalizer, aborter } = opts;
        ...
        performanceLog.debug(
          `create ${type}/${
            documentOrStream.data ? documentOrStream.data.id : 'not-defined'
          } time to complete writer.prepareCreate: ${Date.now() - prepareCreateStart}ms`
        );
      }
      ...
      try {
        ...
        if (typeof beforeFinalize === 'function') {
          ...
          performanceLog.debug(
            `create ${type}/${
              documentOrStream.data ? documentOrStream.data.id : 'not-defined'
            } time to complete beforeFinalize work (create/update card schema resources): ${Date.now() -
              beforeFinalizeStart}ms`
          );
        }
        ...
        if (!indexOnly) {
          ...
          performanceLog.debug(
            `create ${type}/${
              documentOrStream.data ? documentOrStream.data.id : 'not-defined'
            } time to complete finalize: ${Date.now() - finalizeStart}ms`
          );
        } else {
        ...
        performanceLog.debug(
          `create ${type}/${
            documentOrStream.data ? documentOrStream.data.id : 'not-defined'
          } time to complete index save: ${Date.now() - indexSaveStart}ms`
        );
        ...
        performanceLog.debug(
          `create ${type}/${
            documentOrStream.data ? documentOrStream.data.id : 'not-defined'
          } time to complete index invalidation: ${Date.now() - invalidateStart}ms`
        );
      ...
      performanceLog.debug(
        `create ${type}/${
          documentOrStream.data ? documentOrStream.data.id : 'not-defined'
        } time to complete read auth: ${Date.now() - readAuthStart}ms`
      );
      ...
      if (isInternalCard(authorizedDocument.data.type, authorizedDocument.data.id)) {
        ...
        performanceLog.debug(
          `create ${type}/${documentOrStream.data.id} time to complete adapting card to format: ${Date.now() -
            adaptStart}ms`
        );
        performanceLog.debug(
          `create ${type}/${documentOrStream.data.id} total time to create document: ${Date.now() -
            handleCreateStart}ms`
        );
        return card;
      }
      performanceLog.debug(
        `create ${type}/${
          documentOrStream.data ? documentOrStream.data.id : 'not-defined'
        } total time to create document: ${Date.now() - handleCreateStart}ms`
      );
    ...
    async update(session, type, id, document, schema, indexOnly) {
      ...
      if (type === 'cards') {
        let handleCardOperationsStart = Date.now();
        ({ schema, context, internalCard: document, beforeFinalize, afterFinalize } = await this.handleCardOperations(
          session,
          document
        ));
        type = document.data.type;
        performanceLog.debug(
          `update ${type}/${document.data.id} time to complete handleCardOperations: ${Date.now() -
            handleCardOperationsStart}ms`
        );
      }
      ...
      let cleanDocument =
        isInternalCard(type, document.data && document.data.id) && writer.hasCardSupport
          ? this._cleanupCardData(schema, document)
          : this._cleanupBodyData(schema, document.data);
      if (!indexOnly) {
        let opts = await writer.prepareUpdate(session, type, id, cleanDocument, isSchema);
        let { originalDocument, finalDocument, finalizer, aborter } = opts;
        pending = await this.createPendingChange({
          type,
          originalDocument,
      ...
      performanceLog.debug(
        `update ${type}/${document.data.id} time to complete writer.prepareUpdate: ${Date.now() - prepareUpdateStart}ms`
      );
      ...
      try {
        ...
        if (!indexOnly) {
          ...
          context = await this._finalize(pending, type, schema, sourceId);
          performanceLog.debug(
            `update ${type}/${document.data.id} time to complete finalize: ${Date.now() - finalizeStart}ms`
          );
        } else {
          context = pending.finalDocumentContext;
        }
        ...
        await batch.saveDocument(context);
        performanceLog.debug(
          `update ${type}/${document.data.id} time to complete index save: ${Date.now() - indexSaveStart}ms`
        );
        let invalidateStart = Date.now();
        await batch.done();
        performanceLog.debug(
          `update ${type}/${document.data.id} time to complete index invalidation: ${Date.now() - invalidateStart}ms`
        );
        ...
        if (typeof afterFinalize === 'function') {
          ...
          performanceLog.debug(
            `update ${type}/${
              document.data.id
            } time to complete afterFinalize work (delete card schema resources): ${Date.now() - afterFinalizeStart}ms`
          );
        }
        if (newSchema) {
          this.currentSchema.invalidateCache();
      ...
      let authorizedDocument = await context.applyReadAuthorization({ session });
      performanceLog.debug(
        `update ${type}/${document.data.id} time to complete read auth: ${Date.now() - readAuthStart}ms`
      );
      ...
      if (isInternalCard(authorizedDocument.data.type, authorizedDocument.data.id)) {
        ...
        let card = await adaptCardToFormat(schema, session, authorizedDocument, 'isolated', this.searchers);
        performanceLog.debug(
          `update ${type}/${document.data.id} time to complete adapting card to format: ${Date.now() - adaptStart}ms`
        );
        performanceLog.debug(
          `update ${type}/${document.data.id} total time to update document: ${Date.now() - updateStart}ms`
        );
        return card;
      }
      performanceLog.debug(
    ...
    async delete(session, version, type, id, schema, indexOnly) {
      ...
      if (type === 'cards') {
        ...
        performanceLog.debug(
          `delete ${type}/${id} time to complete handleCardOperations: ${Date.now() - handleCardOperationsStart}ms`
        );
      }
      ...
      performanceLog.debug(
        `delete ${type}/${id} time to complete writer.prepareDelete: ${Date.now() - prepareDeleteStart}ms`
      );
      ...
      try {
        ...
        await batch.done();
        performanceLog.debug(
          `delete ${type}/${id} time to complete index invalidation: ${Date.now() - invalidateStart}ms`
        );
    ...
    async _deleteInternalCardResources(session, schema, deletedResources) {
      ...
      let fieldsWithRelatedTypes = schemaToDelete.filter(
        i => i.type === 'fields' && (get(i, 'relationships.related-types.data.length') || 0) > 0
      );
    ...
    async _loadInternalCardSchema(internalOrExternalCard) {
      ...
      return {
        internalCard,
        context,
        schema: await (await this.currentSchema.getSchema()).applyChanges(
          schema.map(document => ({ id: document.id, type: document.type, document }))
        ),
      };
    }
...
class PendingChange {
  constructor({
    ...
    if (schema && searchers) {
      if (originalDocument) {
        this.originalDocumentContext =
          !finalDocument && context
            ? opts.context
            : searchers.createDocumentContext({
