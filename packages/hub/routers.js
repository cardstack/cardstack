...
module.exports = declareInjections(
  ...
  class Routers {
    ...
    async getRoutersInfo() {
      if (this.routerMap && this.applicationCard) {
        return {
          ...
          applicationCard: this.applicationCard,
          routerMapByDepth: this.routerMapByDepth,
        };
      }
    ...
    async getSpace(path, session = {}) {
      ...
      let { params = {}, allowedQueryParams = [], routingCard, matchedRoute, remainingPath, query, routeStack = [] } =
        routeInfo || {};
      ...
      if (!routeInfo) {
        ...
        if (!cards || !cards.length) {
          errorReason = `Can not find card for route '${matchedRoute.path}' with path '${path}' for session ${
            session.type
          }/${session.id} using query ${JSON.stringify(query)}`;
      ...
      log.debug(
        `Routing path '${path}' for session '${session.type}/${session.id}' to space: ${JSON.stringify(space, null, 2)}`
      );
      if (errorReason) {
        log.debug(
          `Routing to path '${path}' for session '${session.type}/${session.id}' resulted in error card. Reason: ${errorReason}`
        );
      }
  ...
  */
    _discoverRouters(schema, routingCardType, routeStack = []) {
      if (routeStack.length > maxRouteDepth) {
        throw new Error(
          `Recursed through more than ${maxRouteDepth} routers when building routing map. Router stack: ${JSON.stringify(
            routeStack,
            null,
            2
          )}`
      ...
      for (let route of router) {
        ...
        if (!path) {
          throw new Error(`The router for content type '${routingCardType}' has a route that is missing a path.`);
        }
        if (path.charAt(0) !== '/') {
          throw new Error(
            `The path of the route for content type '${routingCardType}' at path '${path}' does not begin with '/'.`
          );
        }
        ...
        if (routingCardTypeQueryValue && routingCardTypeQueryValue.charAt(0) === ':') {
          ...
          for (let contentType of schema.getTypes().keys()) {
            ...
            if (parentTypes.includes(contentType)) {
              continue;
            }

            routerMap = routerMap.concat(this._discoverRouters(schema, contentType, childRouteStack));
          }
        } else {
      ...
      routerMap = router
        .filter(route => !pluckedRoutes.includes(route.path))
        .map(route => {
          return {
    ...
    async _getApplicationCard() {
      ...
      try {
        ...
        config = (
          await this.searchers.get(Session.INTERNAL_PRIVILEGED, 'local-hub', 'plugin-configs', '@cardstack/hub')
        ).data;
      } catch (err) {
        if (err.status !== 404) {
          throw err;
    ...
    async _getErrorCard(schema, route, errorCardId) {
      let typeStack =
        route && route.routeStack ? [route.contentType].concat(route.routeStack.map(r => r.contentType)) : [];
      for (let type of typeStack) {
        ...
        if (errorContentType) {
          ...
          if (!(await errorContentType.authorizedReadRealms()).includes('groups/everyone')) {
            log.warn(
              `The error card content-type '${errorType}' does not have a read grant for groups/everyone. Not using this error card.`
            );
          } else {
            let errorCard;
            try {
...
function hasUnconsumedPath(path, cardContext) {
  ...
  if (path.length && path.charAt(0) !== '?' && type === 'application-cards') {
    return true;
  }

  return path.charAt(0) === '?' && decodeURI(path).match(new RegExp(`${type}\\[[^\\]]+\\]=`));
}
...
function buildNameSpacedPathFromRouteStack(routeStack) {
  return (routeStack || []).reduceRight(
    (fullPath, parentRoute) =>
      joinPath(fullPath, parentRoute.path.replace(/(?!:card):([^/?&#]+)/g, `:$1[${parentRoute.contentType}]`)),
    ''
  );
}
