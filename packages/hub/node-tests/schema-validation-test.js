...
describe('schema/validation', function() {
  ...
  before(async function() {
    ...
    ephemeralDataSource = factory.addResource('data-sources').withAttributes({
      sourceType: '@cardstack/ephemeral',
      params: {
        initialModels: [{ type: 'hello', id: 'world' }],
      },
    });

    factory.addResource('plugin-configs', '@cardstack/hub').withRelated('defaultDataSource', ephemeralDataSource);

    let articleType = factory.addResource('content-types', 'articles').withRelated('data-source', ephemeralDataSource);

    let titleField = factory
      .addResource('fields', 'title')
      .withAttributes({ fieldType: '@cardstack/core-types::string' });

    let coverImageField = factory
      .addResource('fields', 'cover-image')
      .withAttributes({ fieldType: '@cardstack/core-types::string' });

    let coverVideoField = factory
      .addResource('fields', 'cover-video')
      .withAttributes({ fieldType: '@cardstack/core-types::string' });
    ...
    let publishedDateField = factory.addResource('fields', 'published-date').withAttributes({
    ...
    articleType.withRelated('fields', [
      ...
      coverImageField,
      coverVideoField,
      factory.addResource('fields', 'colors').withAttributes({
      ...
      factory
        ...
        .withRelated('relatedTypes', [factory.addResource('content-types', 'images')]),
      factory
        .addResource('fields', 'detail-images')
        .withAttributes({
    ...
    factory
      ...
      .withRelated('fields', [
        factory.addResource('fields', 'event-title').withAttributes({ fieldType: '@cardstack/core-types::string' }),
      ]);
    ...
    factory.addResource('content-types', 'things-with-defaults').withRelated('fields', [
      factory
        .addResource('fields', 'timestamp')
        .withAttributes({
          fieldType: '@cardstack/core-types::date',
        })
        .withRelated('defaultAtUpdate', factory.addResource('default-values').withAttributes({ value: 'now' })),
      factory
