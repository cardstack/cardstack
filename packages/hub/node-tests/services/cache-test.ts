...
describe('CardCache', function () {
  ...
  it('.writeAsset', async function () {
    ...
    expect(cache.entryExists('assets', cardURL, filename), 'Asset is placed in assets/<encoded-card-url>/<filename>').to
      .be.true;
    expect(cache.entryExists('node', cardURL, filename), 'Symlink is created in node/<encoded-card-url>/<filename>').to
      .be.true;
    expect(
