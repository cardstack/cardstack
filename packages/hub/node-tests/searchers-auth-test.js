...
describe('hub/searchers/auth', function() {
  ...
  async function setup(fn) {
    ...
    factory
      ...
      .withRelated(
        'author',
        factory.addResource('authors', 'arthur').withAttributes({
  ...
  it('matches a group grant', async function() {
    await setup(factory => {
      ...
      factory.addResource('groups', 'cool-kids').withAttributes({
  ...
  it('matches a multi-group grant', async function() {
    await setup(factory => {
      factory
        ...
        .withAttributes({ mayReadResource: true, mayReadFields: true })
        .withRelated('who', [
          { type: 'groups', id: 'cool-kids' },
          { type: 'groups', id: 'famous-people' },
        ]);
      factory.addResource('groups', 'cool-kids').withAttributes({
        searchQuery: { filter: { type: { exact: 'authors' }, name: { exact: 'Arthur Faulkner' } } },
      });
      factory.addResource('groups', 'famous-people').withAttributes({
        searchQuery: { filter: { type: { exact: 'authors' }, name: { exact: 'Arthur Faulkner' } } },
      });
    });
    let doc = await searchers.search(sessions.create('authors', 'arthur'), { filter: { type: 'posts' } });
  ...
  it('stops matching a group grant when the user is deleted', async function() {
    ...
    await env
      .lookup('hub:writers')
      .delete(Session.INTERNAL_PRIVILEGED, user.data.meta.version, user.data.type, user.data.id);
  ...
  it('reacts when a grant is updated', async function() {
    await setup(factory => {
      factory.addResource('groups', 'not-cool-kids').withAttributes({
