...
describe('schema/auth/write', function() {
  ...
  beforeEach(async function() {
    ...
    factory.addResource('content-types', 'articles').withRelated('fields', [
      ...
      factory
        ...
        .withRelated('defaultAtCreate', factory.addResource('default-values').withAttributes({ value: 0 })),
      factory
        .addResource('fields', 'reviewed')
        .withAttributes({
          fieldType: '@cardstack/core-types::boolean',
        })
        .withRelated('defaultAtUpdate', factory.addResource('default-values').withAttributes({ value: false })),
      factory.addResource('fields', 'misc').withAttributes({
        fieldType: '@cardstack/core-types::object',
      }),
      factory.addResource('computed-fields', 'creators').withAttributes({
        computedFieldType: 'sample-computed-fields::authorized-user',
        params: { type: 'puppies', id: 'vanGogh' },
      }),
      factory
    ...
    // These need to be in the index in order for the Model#getRelated
    // to load these relationship models when computing the grant matching
    factory.addResource('puppies', 'vanGogh');
    factory.addResource('puppies', 'jojo');
    factory.addResource('authors', '1');
    factory.addResource('authors', '2');
    factory.addResource('authors', '123');
    factory.addResource('test-users', '0');

    factory.addResource('content-types', 'events').withRelated('fields', [factory.getResource('fields', 'title')]);
  ...
  function schemaModels(factory) {
    return factory.getModels().filter(i => !['puppies', 'authors', 'test-users'].includes(i.type));
  }
  ...
  it("everyone grant allows creation when there's no session", async function() {
    ...
    factory
      .addResource('grants')
      .withAttributes({ mayCreateResource: true })
      .withRelated('who', [everyone]);
    let schema = await loader.loadFrom(schemaModels(factory));
    let action = await create({
      type: 'articles',
  ...
  it('user-provided id approved with a grant', async function() {
    factory
      .addResource('grants')
      .withAttributes({ mayCreateResource: true, mayReadResource: true })
      .withRelated('who', [everyone]);
    factory
      .addResource('grants')
      .withAttributes({ mayWriteFields: true })
      .withRelated('who', [everyone])
      .withRelated('fields', [factory.getResource('fields', 'id')]);
  ...
  it('id-dependent grant allows creation', async function() {
    ...
    factory
      .addResource('grants')
      .withAttributes({ mayCreateResource: true, mayWriteFields: true })
  ...
  it('attribute-dependent grant using computed allows creation', async function() {
    allReadable();
    factory
      .addResource('grants')
      .withAttributes({ mayCreateResource: true, mayWriteFields: true })
    ...
    let session = makeSession(schema, { type: 'puppies', id: 'vanGogh' });
    let errors = await schema.validationErrors(action, { session });
    expect(errors).deep.equal([]);
  });

  it('attribute-dependent grant using computed rejects creation', async function() {
    allReadable();
    factory
      .addResource('grants')
      .withAttributes({ mayCreateResource: true, mayWriteFields: true })
    ...
    let session = makeSession(schema, { type: 'puppies', id: 'jojo' });
    let errors = await schema.validationErrors(action, { session });
    expect(errors).collectionContains({
      status: 401,
      detail: 'You may not create this resource',
  ...
  it('forbids update when attribute-dependent grant matches only the final state', async function() {
    ...
    factory
      .addResource('grants')
      .withAttributes({ mayUpdateResource: true, mayWriteFields: true })
      .withRelated('who', [{ type: 'fields', id: 'author' }]);
    let schema = await loader.loadFrom(schemaModels(factory));
    let action = await update(
      {
  ...
  it('forbids deletion when attribute-dependent grant does not match', async function() {
    factory
      .addResource('grants')
      .withAttributes({ mayDeleteResource: true, mayReadFields: true })
      .withRelated('who', [{ type: 'fields', id: 'author' }]);
  ...
  it('allows deletion when attribute-dependent grant matches', async function() {
    factory
      .addResource('grants')
      .withAttributes({ mayDeleteResource: true })
      .withRelated('who', [{ type: 'fields', id: 'author' }]);
  ...
  it('approves via a field-specific grant', async function() {
    ...
    factory
      ...
      .withAttributes({ mayUpdateResource: true, mayWriteFields: true })
      .withRelated('who', [everyone])
      .withRelated('fields', [factory.getResource('fields', 'coolness')]);
    let schema = await loader.loadFrom(schemaModels(factory));
    let action = await update(
      {
        type: 'articles',
  ...
  it('approves via a field-specific grant an update to a relationship', async function() {
    ...
    factory
      ...
      .withAttributes({ mayUpdateResource: true, mayWriteFields: true })
      .withRelated('who', [everyone])
      .withRelated('fields', [factory.getResource('fields', 'author')]);
    let schema = await loader.loadFrom(schemaModels(factory));
    await create({
      id: '1',
      type: 'authors',
  ...
  it('approves via a field-specific grant an update to an object type field', async function() {
    ...
    factory
      ...
      .withAttributes({ mayUpdateResource: true, mayWriteFields: true })
      .withRelated('who', [everyone])
      .withRelated('fields', [factory.getResource('fields', 'misc')]);
    let schema = await loader.loadFrom(schemaModels(factory));
    let action = await update(
      {
        type: 'articles',
  ...
  it('approves field write at update via unchanged value', async function() {
    ...
    factory
      .addResource('grants')
      .withAttributes({ mayUpdateResource: true })
      .withRelated('who', [everyone]);
    let schema = await loader.loadFrom(schemaModels(factory));
    let action = await update(
      {
