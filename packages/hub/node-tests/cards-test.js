...
async function convertToExternalFormat(internalCard) {
  let externalCard = await adaptCardToFormat(
    ...
    env.session,
    cloneDeep(internalCard),
    'isolated',
    env.lookup('hub:searchers')
  );
...
function assertCardOnDisk() {
  let browserAssets = ['embedded.css', 'embedded.js', 'embedded.hbs', 'isolated.css', 'isolated.js', 'isolated.hbs'];

  ['card.js', 'package.json']
    .concat(browserAssets)
    .map(file =>
      expect(pathExistsSync(join(cardsDir, 'local-hub', 'millenial-puppies', file))).to.equal(true, `${file} exists`)
    );
...
function assertIsolatedCardMetadata(card) {
  ...
  expect(data.attributes['field-order']).to.eql(['title', 'author', 'body', 'tag-names', 'tags']);
...
function assertEmbeddedCardMetadata(card) {
  ...
  expect(data.attributes['embedded-metadata-summary']).to.be.undefined; // this one shouldn't leak into the external format
  expect(data.attributes['internal-fields-summary']).to.be.undefined; // this one shouldn't leak into the external format
  expect(data.attributes['field-order']).to.eql(['title', 'author', 'tag-names']);
  expect(Object.keys(data.attributes['metadata-summary'])).to.have.members(['title', 'author', 'tag-names']);

  expect(includedIdentifiers).to.not.include.members(['tags/millenials', 'tags/puppies', 'tags/belly-rubs']);
  expect(includedIdentifiers).to.include.members(['cards/local-hub::van-gogh']);
...
function assertCardModels(card) {
  let { data, included } = card;
  let includedIdentifiers = included.map(i => `${i.type}/${i.id}`);
  expect(data.relationships.model.data).to.eql({
    type: 'local-hub::millenial-puppies',
    id: 'local-hub::millenial-puppies',
  });
...
function assertCardSchema(card) {
  ...
  // to include anyways...
  expect(includedIdentifiers).to.not.include.members(['content-types/local-hub::millenial-puppies']);
...
describe('hub/card-services', function() {
  ...
  describe('card validation', function() {
    ...
    it('does not allow a card to have a belongs-to relationship to a foreign internal model', async function() {
      ...
      expect(error.source).to.eql({
        pointer: `/data/relationships/local-hub::foreign-internal-belongs-to-relationship::related-thing/data`,
      });
    });
    ...
    it('does not allow a card to have a has-many relationship to a foreign internal model', async function() {
      ...
      expect(error.message).to.match(/has a relationship to a foreign internal model/);
      expect(error.source).to.eql({
        pointer: `/data/relationships/local-hub::foreign-internal-has-many-relationship::related-things/data/0`,
      });
    });
  });
  ...
  describe('writing cards', function() {
    ...
    it('does not allow missing card model when creating card', async function() {
      ...
      missingModel.included = missingModel.included.filter(
        i => `${i.type}/${i.id}` !== 'local-hub::millenial-puppies/local-hub::millenial-puppies'
      );
    ...
    it('does not allow a card to be created with a model specified but missing in the included resource', async function() {
      ...
      try {
        ...
        badArticle.included = badArticle.included.filter(
          i => `${i.type}/${i.id}` !== 'local-hub::millenial-puppies/local-hub::millenial-puppies'
        );
    ...
    it("can add a field to a card's schema", async function() {
      ...
      expect(data.attributes['field-order']).to.eql(['title', 'author', 'body', 'tag-names', 'tags', 'editor']);
    ...
    it("can remove a field from the card's schema", async function() {
      ...
      expect(data.attributes['field-order']).to.eql(['title', 'author', 'tag-names', 'tags']);
    ...
    it('can update field order', async function() {
      let card = await cardServices.create(env.session, externalArticleCard);
      card.data.attributes['field-order'] = ['body', 'author', 'tags', 'tag-names', 'title'];
      let { data } = await cardServices.update(env.session, 'local-hub::millenial-puppies', card);
      expect(data.attributes['field-order']).to.eql(['body', 'author', 'tags', 'tag-names', 'title']);
      card = await cardServices.get(env.session, 'local-hub::millenial-puppies', 'isolated');
      data = card.data;
      expect(data.attributes['field-order']).to.eql(['body', 'author', 'tags', 'tag-names', 'title']);
    });
    ...
    it('does not allow missing card model when updating card', async function() {
      ...
      card.included = card.included.filter(
        i => `${i.type}/${i.id}` !== 'local-hub::millenial-puppies/local-hub::millenial-puppies'
      );
  ...
  describe('get card', function() {
    ...
    it('has a base-card', async function() {
      ...
      let model = included.find(
        i => `${i.type}/${i.id}` === 'local-hub::@cardstack/base-card/local-hub::@cardstack/base-card'
      );
  ...
  describe('card read authorization', function() {
    ...
    beforeEach(async function() {
      ...
      factory
        ...
        .withRelated('types', [{ type: 'content-types', id: 'local-hub::millenial-puppies' }])
        .withRelated('fields', [{ type: 'fields', id: 'local-hub::millenial-puppies::title' }])
        .withAttributes({
      ...
      factory
        .addResource('grants')
        .withRelated('who', [{ type: 'test-users', id: 'allowed-user' }])
        .withRelated('types', [{ type: 'content-types', id: 'local-hub::van-gogh' }])
        .withAttributes({
          mayReadResource: true,
          mayReadFields: true,
        });
    ...
    describe('get()', function() {
      ...
      it('does not contain included resource for card metadata relationship that the session does not have read authorization', async function() {
        ...
        let includedIdentifiers = included.map(i => `${i.type}/${i.id}`);
        expect(includedIdentifiers).to.not.include.members(['cards/local-hub::van-gogh']);
      });
    ...
    describe.skip('TODO search()', function() {
      ...
      it('does not return card metadata that the session does not have read authorization', async function() {
        let {
          data: [card],
          included: [model],
        } = await cardServices.search(restrictedUser, 'isolated', {
          filter: {
      ...
      it('does return card metadata that the session has read authorization', async function() {
        let {
          data: [card],
          included: [model],
        } = await cardServices.search(allowedUser, 'isolated', {
          filter: {
            id: { exact: 'local-hub::millenial-puppies' },
            type: { exact: 'cards' },
          },
  ...
  describe.skip('search for card', function() {
    beforeEach(async function() {
      ...
      factory.addResource('data-sources', 'stub-card-project').withAttributes({
        sourceType: 'stub-card-project',
        params: {
          cardSearchResults: [internalArticleCard, userCard],
        },
      });
    ...
    it("will load a card implicitly when a searcher's search() hook returns a card document in embedded format", async function() {
      let {
        data: [article],
      } = await cardServices.search(env.session, 'embedded', {
        filter: {
          type: { exact: 'cards' },
        },
      });
  ...
  describe('card adoption', function() {
    ...
    it('can get a card that uses adoption in an isolated format', async function() {
      ...
      expect(adoptedCreateResponse).to.eql(
        adopted,
        'the card returned from the create() is the same as the card returned from get()'
      );
      expect(adopted.data.relationships['adopted-from'].data).to.eql({
        type: 'cards',
        id: externalArticleCard.data.id,
      });
      expect(adopted.data.attributes['adoption-chain']).to.eql([externalArticleCard.data.id]);
      expect(adopted.data.attributes['field-order']).to.eql(['title', 'author', 'body', 'tag-names', 'tags', 'yarn']);
    ...
    it('can get a card that uses adoption in an embedded format', async function() {
      ...
      expect(adopted.data.attributes['adoption-chain']).to.eql([externalArticleCard.data.id]);
      expect(adopted.data.attributes['field-order']).to.eql(['title', 'author', 'tag-names', 'yarn']);
    ...
    it('can get an isolated card that uses multiple levels of adoption', async function() {
      ...
      expect(adopted.data.attributes['adoption-chain']).to.eql([genXKittens.data.id, externalArticleCard.data.id]);
    ...
    it('can get an embedded card that uses multiple levels of adoption', async function() {
      ...
      expect(adoptedCreateResponse).not.to.eql(
        adopted,
        'the card returned from the create() is the same as the card returned from get()'
      );
      expect(adopted.data.relationships['adopted-from'].data).to.eql({
      ...
      expect(adopted.data.attributes['adoption-chain']).to.eql([genXKittens.data.id, externalArticleCard.data.id]);
      expect(adopted.data.attributes['field-order']).to.eql(['title', 'author', 'tag-names', 'yarn', 'cuteness']);
    ...
    it('can update adopted field order', async function() {
      ...
      data = card.data;
      expect(data.attributes['field-order']).to.eql(['body', 'author', 'tags', 'tag-names', 'title', 'yarn']);
    });
    ...
    it('field order includes any fields added as a result of document invalidation', async function() {
      ...
      let card = await cardServices.create(env.session, genXKittens);
      card.data.attributes['field-order'] = ['body', 'author', 'tags', 'tag-names', 'yarn', 'title'];
      card = await cardServices.update(env.session, card.data.id, card);
      expect(card.data.attributes['field-order']).to.eql(['body', 'author', 'tags', 'tag-names', 'yarn', 'title']);
    ...
    it('can remove an adopted field via an upstream change', async function() {
      ...
      parentCard.data.relationships.fields.data = parentCard.data.relationships.fields.data.filter(
        i => `${i.type}/${i.id}` !== `fields/body`
      );
      ...
      expect(card.data.attributes['field-order']).to.eql(['title', 'author', 'tag-names', 'tags', 'yarn']);
      expect(Object.keys(card.data.attributes['metadata-summary'])).to.have.members([
        'title',
        'author',
        'tag-names',
    ...
    it('can change the adopted-from relationship to adopt from a different parent', async function() {
      let factory = new JSONAPIFactory();
      let card = await cardServices.create(
        env.session,
        factory.getDocumentFor(
          factory
            .addResource('cards', 'local-hub::child')
            .withRelated('fields', [
            ...
            .withRelated(
              'model',
              factory.addResource('local-hub::child', 'local-hub::child').withAttributes({
      ...
      expect(updatedCard.included.find(i => `${i.type}/${i.id}` === `cards/${externalArticleCard.data.id}`)).to.be.not
        .ok;
      expect(updatedCard.included.find(i => `${i.type}/${i.id}` === `cards/${externalUserCard.data.id}`)).to.be.ok;
      expect(updatedCard.data.relationships['adopted-from'].data).to.eql({
        type: 'cards',
        id: externalUserCard.data.id,
      });
