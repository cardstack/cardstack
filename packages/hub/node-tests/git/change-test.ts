...
import { GitConflict } from '@cardstack/git-realm-card/lib/git';
import { inRepo, commitOpts, makeRepo } from './support';
import { FileNotFound, OverwriteRejected } from '@cardstack/git-realm-card/lib/tree';
...
describe('git/change', function() {
  ...
  it('can patch a file', async function() {
    ...
    file.setContent('The original was: ' + Buffer.from(originalBuffer as Uint8Array)!.toString('utf8'));
    head = await change.finalize(commitOpts());
