...
async function withGrants(fn) {
  ...
  let schema = await baseSchema.applyChanges(
    factory.getModels().map(model => ({ type: model.type, id: model.id, document: model }))
  );
  ...
  factory
    ...
    .withRelated('fields', [
      ...
      factory
        ...
        .withRelated('related-types', [factory.addResource('content-types', 'tags')]),
      factory
        ...
        .withRelated('related-types', [
          factory.addResource('content-types', 'authors').withRelated('fields', [
            ...
            factory
              .addResource('fields', 'flavor')
              .withAttributes({
                fieldType: '@cardstack/core-types::belongs-to',
              })
              .withRelated('related-types', [factory.addResource('content-types', 'flavors')]),
          ]),
        ]),
      factory
        .addResource('fields', 'collaborators')
        .withAttributes({
          fieldType: '@cardstack/core-types::has-many',
        })
...
describe('schema/auth/read', function() {
  before(async function() {
    ...
    factory
      ...
      .withRelated(
        'author',
        factory
          .addResource('authors', '1')
          .withAttributes({
            name: 'Arthur Faulkner',
          })
          .withRelated('flavor', factory.addResource('flavors', 'vanilla'))
      )
      .withRelated('tags', [factory.addResource('tags', 'one'), factory.addResource('tags', 'two')])
...

    {
      ...
      factory
        ...
        .withRelated('fields', [factory.getResource('fields', 'title'), factory.getResource('fields', 'author')])
        .withAttributes({
          mayReadResource: true,
          mayReadFields: true,
        });
...

    {
      ...
      factory
        .addResource('grants')
        .withRelated('who', [{ type: user.type, id: user.id }])
        .withRelated('types', [factory.getResource('content-types', 'posts')])
        .withAttributes({
          mayReadResource: true,
          mayReadFields: true,
        });
      factory
        ...
        .withRelated('types', [factory.getResource('content-types', 'authors')])
        .withAttributes({
          mayReadResource: true,
        });
    }
  ...
  it('returns nothing when user has a grant with a different type', async function() {
    ...
    let { schema, session } = await withGrants(factory => {
      factory
        .addResource('grants')
        .withRelated('types', [{ type: 'content-types', id: 'authors' }])
        .withAttributes({
          mayReadResource: true,
        });
    });
  ...
  it('keeps linked includes in collection document', async function() {
    ...
    let { schema, session } = await withGrants(factory => {
      factory
        ...
        .withRelated('fields', [{ type: 'fields', id: 'author' }])
        .withAttributes({
          mayReadResource: true,
          mayReadFields: true,
        });
  ...
  it('when adjusting includes, keeps valid has-many relationships', async function() {
    ...
    let { schema, session } = await withGrants(factory => {
      factory
        ...
        .withRelated('fields', [{ type: 'fields', id: 'tags' }])
        .withAttributes({
          mayReadResource: true,
          mayReadFields: true,
        });
  ...
  it('removes unlinked second-level authorized includes', async function() {
    ...
    let { schema, session } = await withGrants(factory => {
      factory
        ...
        .withRelated('fields', [{ type: 'fields', id: 'flavor' }])
        .withAttributes({
          mayReadResource: true,
          mayReadFields: true,
        });
