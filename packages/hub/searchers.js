  ...
  class Searchers {
    ...
    async get(session, source, type, id, opts = {}) {
      if (source !== localHubSource) {
        throw new Error(
          `You specified the source: '${source}' in Searchers.get(). Currently the cardstack hub does not support non-local hub sources.`
        );
      }
    ...
    async _setupWorkers() {
      ...
      await this.jobQueue.subscribe('hub/searchers/cache', async ({ data: { maxAge, upstreamDoc } }) => {
        this._cachingPromise = Promise.resolve(this._cachingPromise).then(() =>
          this._cacheDocument(upstreamDoc, maxAge)
        );
