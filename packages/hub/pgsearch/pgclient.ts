import { Pool, Client, QueryArrayConfig, QueryArrayResult, QueryResultRow, QueryConfig, QueryResult } from "pg";
...
const config = postgresConfig({
  database: `pgsearch_${process.env.PGSEARCH_NAMESPACE}`
});
...
export default class PgClient {
  ...
  private async migrateDb() {
    ...
    try {
      ...
      if (!response.rows[0].has_database) {
        await client.query(
          `create database ${safeName(config.database)}`
        );
      }
    } finally {
      client.end();
  ...
  static async deleteSearchIndexIHopeYouKnowWhatYouAreDoing() {
    ...
    try {
      await client.connect();
      await client.query(
        `drop database if exists ${safeName(config.database)}`
      );
    } finally {
      client.end();
    }
...
class Batch {
  ...
  async save(card: CardWithId) {
    ...
    await this.client.query(
      queryToSQL(upsert("documents", "documents_pkey", row))
    );
