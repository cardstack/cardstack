...
module('Integration | Models', function(hooks) {
  ...
  let scenario = new Fixtures({
    create(factory) {
      ...
      factory
        ...
        .withRelated('author', factory.addResource('authors').withAttributes({ nickname: 'Author of Second' }));
      factory
  ...
  test('it can query for the base type', async function(assert) {
    let models = await run(() => this.store.query('cardstack-card', { filter: { title: 'world' } }));
    assert.equal(models.get('length'), 1);
    assert.equal(models.objectAt(0).get('id'), '1');
  });
