...
function internalCardDocument(titleValue, id = 'local-hub::test-card') {
    let factory = new JSONAPIFactory();
    return factory.getDocumentFor(factory.addResource(id, id)
...
describe('git/indexer', function () {
    ...
    beforeEach(async function () {
        ...
        assertNoCard = async function (id) {
            try {
    ...
    it('processes first empty branch', async function () {
        let { head } = await support_1.makeRepo(root);
        await start();
    ...
    it('indexes newly added document', async function () {
        let { repo, head } = await support_1.makeRepo(root);
        await start();
    ...
    it('ignores newly added document with malformed json', async function () {
        ...
        head = await change.finalize(support_1.commitOpts());
        await logger_1.default.expectWarn(/Ignoring record with invalid json at contents\/articles\/hello-world.json/, async () => {
            await indexer.update();
        });
        let indexerState = await client.loadMeta({ branch: 'master', id: dataSource.id });
        expect(indexerState.commit).to.equal(head);
    ...
    it('replaces unrelated content it finds in the search index', async function () {
        ...
        await support_1.makeRepo(repos + '/left', {
        ...
        await support_1.makeRepo(repos + '/right', {
        ...
        await change.finalize(support_1.commitOpts());
        await logger_1.default.expectWarn(/Unable to load previously indexed commit/, async () => {
...
describe('git/indexer failures', function () {
    ...
    it("makes a repo if one doesn't already exist", async function () {
        expect(fs_1.default.existsSync(root + '/repo-to-be-created')).is.ok;
    });
});
