...
def create_rule(
    ...
    if fake_data_table2 is not None:
        con.execute(
            f"""create table {table_names[1]} as select * from fake_data_table2"""
        )
    ...
    def run_query(self, tables_names, vars):
        ...
        if test == "compound_parameters":
            vars = [
                "0x52031d287Bb58E26A379A7Fec2c84acB54f54fe3"
            ]  # [token, num_of_blocks_per_month, interest_rate_monthly]
...
def test_compound_parameters(monkeypatch):
    ...
    fake_data_safe_owner = pd.DataFrame([{"owner": "owner1", "safe": "safe1"}])
...
def test_partial_rewards(monkeypatch):
    ...
    rule = create_rule(
        monkeypatch, "partial_rewards", table_names, fake_data_compound_parameter
    )
...
def test_base_rewards(monkeypatch):
    ...
    rule = create_rule(
        monkeypatch, "base_rewards", table_names, fake_partial_rewards_data
    )
    result = rule.run(200, "0x0")
    assert len(result) == 2
    for i in range(len(result)):
...
def test_intermediate_rewards(monkeypatch):
    ...
    rule = create_rule(
        monkeypatch, "intermediate_rewards", table_names, fake_partial_rewards_data
    )
    result = rule.run(200, "0x0")
    # assert len(result) == 3
    ...
    for i in range(len(fake_partial_rewards_data)):
        partial_reward = fake_partial_rewards_data.where(
            fake_partial_rewards_data["id"] == i
        )["partial_reward"][i]
        safe = fake_partial_rewards_data.where(fake_partial_rewards_data["id"] == i)[
            "safe"
        ][i]
    ...
    for i in range(len(result)):
        ...
        inter_rewards = result.where(result["owner"] == owner_idx)[
...
def test_total_monthly_rewards(monkeypatch):
    ...
    fake_data_partial_rewards = pd.DataFrame(
        [
            {"safe": "safe1", "intermediate_rewards": 191.66},
            {"safe": "safe2", "intermediate_rewards": -91.666},
            {"safe": "safe3", "intermediate_rewards": 0},
        ]
    )

    fake_data_base_rewards = pd.DataFrame(
        [
            {"vid": 0, "safe": "safe1", "owner": "owner1", "base_reward": 500},
            {"vid": 1, "safe": "safe2", "owner": "owner2", "base_reward": 750},
            {"vid": 2, "safe": "safe3", "owner": "owner3", "base_reward": 7500},
        ]
    )
    ...
    rule = create_rule(
        ...
        "monthly_rewards",
        table_names,
        fake_data_partial_rewards,
        fake_data_base_rewards,
    )
    result = rule.run(200, "0x0")
    assert len(result) == 3
    for i in range(len(result)):
        base_reward = result.where(result["vid"] == i)["base_reward"][i]
        intermediate_reward = result.where(result["vid"] == i)["intermediate_rewards"][
            i
        ]
