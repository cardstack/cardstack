...
class DummyRule(Rule):
    ...
    def set_user_defined_parameters(
        self, token, base_reward, duration, hist_balance_threshold
    ):
        self.token = token
    ...
    def sql_2(self, tables_names):
        ...
        spend_accumulation_table = self._get_table_query(
            "spend_accumulation", tables_names[0], self.start_block, self.end_block
        )

        safe_owner_table = self._get_table_query(
            "safe_owner", tables_names[1], self.start_block, self.end_block
        )

        print(safe_owner_table)
        # If any of the tables is empty return a none string
        if (
            safe_owner_table == "parquet_scan([])"
            or spend_accumulation_table == "parquet_scan([])"
        ):
            return None
    ...
    def run(self, payment_cycle: int, reward_program_id: str):
        vars = [self.start_block, self.end_block, self.hist_balance_threshold]
