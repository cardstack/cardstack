...
export default class MusicDetailCardComponent extends Component {
  ...
  get recordingDetails() {
    ...
    return [
      ...
      {
        title: 'main artist',
        value: this.model.artists || [
          { type: 'participant', title: this.model.artist },
        ],
        type: 'collection',
        component: 'cards/artist',
        search: (q) =>
          (this.model.selectableArtists || []).filter((a) =>
  ...
  get musicalWork() {
    ...
    return {
      ...
      search: (q) =>
        (this.model.selectableWorks || []).filter((a) =>
          a.title.toLowerCase().includes(q.toLowerCase())
        ),
    };
  }
