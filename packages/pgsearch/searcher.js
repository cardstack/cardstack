...
const { queryToSQL, param, every, addExplicitParens, any, separatedByCommas } = require('./util');
  ...
  class Searcher {
    ...
    async get(session, type, id) {
      let response = await this.client.query(
        'select pristine_doc from documents where type=$1 and id=$2 and (expires is null or expires > now())',
        [type, id]
      );
    ...
    async search(session, { filter, sort, page, queryString }) {
      ...
      let conditions = [['realms && ', param(realms)], ['expires is null or expires > now()']];
      ...
      let totalResponsePromise = this.client.query(
        queryToSQL([`select count(*) from documents where`, ...every(conditions)])
      );
      ...
      let query = [
        `select`,
        ...sorts.cursorColumns(),
        `, pristine_doc from documents where`,
        ...every(conditions),
    ...
    _buildQueryExpression(schema, segments, errorHint, partialPath, currentContext, insideHasMany) {
      ...
      if (rest.length > 0) {
        ...
        if (field.fieldType === '@cardstack/core-types::has-many') {
          ...
          return this._buildQueryExpression(
            schema,
            rest,
            errorHint,
            `${partialPath}${first}.`,
            ['(', ...currentContext, ')->', param(field.id)],
            insideHasMany
          );
        }
      } else {
    ...
    fieldFilter(schema, key, value) {
      ...
      if (typeof value === 'string') {
        ...
        if (isPlural) {
          return [
            ...
            ...leafField.buildValueExpression(value),
            `)`,
          ];
        } else {
          return [
      ...
      if (value.prefix) {
        ...
        return [
          ...
          ...expression,
          `) @@ to_tsquery('english',`,
          ...leafField.buildValueExpression(param),
          `)`,
        ];
      }
...
class Sorts {
  ...
  cursorColumns() {
    return separatedByCommas(
      this._sorts.map(({ expression }, index) => {
