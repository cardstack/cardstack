...
contract('SampleToken', function(accounts) {
  ...
  describe('private blockchain sanity checks', function() {
    ...
    it('should transfer the token', async function() {
      ...
      expect(accountOneEndingBalance).to.equal(
        accountOneStartingBalance - amount,
        'Amount was correctly taken from the sender',
      );
      expect(accountTwoEndingBalance).to.equal(
        accountTwoStartingBalance + amount,
        'Amount was correctly sent to the receiver',
      );
  ...
  describe('ethereum-indexer', function() {
    ...
    async function setup() {
      ...
      dataSource = factory.addResource('data-sources', contractName).withAttributes({
    ...
    it('can generate schema from ERC-20 contract`s ABI', async function() {
      ...
      schema = await env
        ...
        .get(env.session, 'master', 'content-types', 'sample-token-balance-ofs');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-custom-buyer-limits');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-vesting-schedules');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vesting-schedule-fully-vested-amount');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vesting-schedule-fully-vested-amount',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'grants', 'sample-token-vesting-schedule-grant');
      expect(schema).to.deep.equal({
        data: {
          type: 'grants',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-approved-buyers');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-transfer-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
          id: 'sample-token-transfer-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'grants', 'sample-token-transfer-events-grant');
      expect(schema).to.deep.equal({
        data: {
          type: 'grants',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-token-frozen-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
          id: 'sample-token-token-frozen-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'grants', 'sample-token-token-frozen-events-grant');
      expect(schema).to.deep.equal({
        data: {
          type: 'grants',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'sample-token-token-frozen-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'sample-token-token-frozen-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-mint-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
          id: 'sample-token-mint-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-white-list-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
          id: 'sample-token-white-list-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'grants', 'sample-token-white-list-events-grant');
      expect(schema).to.deep.equal({
        data: {
          type: 'grants',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'content-types', 'sample-token-vested-token-grant-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'content-types',
          id: 'sample-token-vested-token-grant-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'grants', 'sample-token-vested-token-grant-events-grant');
      expect(schema).to.deep.equal({
        data: {
          type: 'grants',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'sample-token-vested-token-grant-events');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'sample-token-vested-token-grant-events',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-beneficiary');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-beneficiary',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-start-date');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-start-date',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-cliff-sec');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-cliff-sec',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-duration-sec');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-duration-sec',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-fully-vested-amount');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-fully-vested-amount',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-revoke-date');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-revoke-date',
      ...
      schema = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'fields', 'vested-token-grant-event-is-revocable');
      expect(schema).to.deep.equal({
        data: {
          type: 'fields',
          id: 'vested-token-grant-event-is-revocable',
    ...
    it('indexes mapping entry that contains multiple return values', async function() {
      let { logs: events } = await token.grantVestedTokens(
        accountOne,
        100,
        1000000000,
        1000500000,
      ...
      let vestingSchedule = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-vesting-schedules', accountOne);

      expect(vestingSchedule.data.attributes['ethereum-address']).to.not.equal(
        vestingSchedule.data.id,
        'the case between the addresses is different',
      );
      vestingSchedule.data.attributes['ethereum-address'] = vestingSchedule.data.attributes[
        'ethereum-address'
      ].toLowerCase();
    ...
    it('indexes mapping entry content types when a contract fires an ethereum event', async function() {
      ...
      let accountOneLedgerEntry = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountOne);
      expect(accountOneLedgerEntry.data.attributes['ethereum-address']).to.not.equal(
        accountOneLedgerEntry.data.id,
        'the case between the addresses is different',
      );
      accountOneLedgerEntry.data.attributes['ethereum-address'] = accountOneLedgerEntry.data.attributes[
        'ethereum-address'
      ].toLowerCase();
      ...
      let accountTwoLedgerEntry = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountTwo);
      expect(accountTwoLedgerEntry.data.attributes['mapping-number-value']).to.equal(
        '10',
        'the token balance is correct',
      );

      let transferEvent1 = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-transfer-events', transferEvent1Id);
      ...
      let transferEvent2 = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-transfer-events', transferEvent2Id);
      ...
      expect(transferEvent2.data.attributes['block-number']).to.equal(
        transferEvents[0].blockNumber,
        'the block number is correct',
      );
      expect(transferEvent2.data.attributes['transaction-id']).to.equal(
      ...
      expect(transferEvent2.data.attributes['transfer-event-from']).to.equal(
        accountOne,
        'the transaction event from address is correct',
      );
      expect(transferEvent2.data.attributes['transfer-event-to']).to.equal(
        accountTwo,
        'the transaction event to address is correct',
      );
      expect(transferEvent2.data.attributes['transfer-event-value']).to.equal(
        '10',
        'the transaction event value is correct',
      );
    });
    ...
    it('indexes mapping entry content with different field types for the same event', async function() {
      ...
      let accountOneApprovedBuyer = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-approved-buyers', accountOne);
      accountOneApprovedBuyer.data.attributes['ethereum-address'] = accountOneApprovedBuyer.data.attributes[
        'ethereum-address'
      ].toLowerCase();
      ...
      let accountOneCustomBuyerLimit = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-custom-buyer-limits', accountOne);
      accountOneCustomBuyerLimit.data.attributes['ethereum-address'] = accountOneCustomBuyerLimit.data.attributes[
        'ethereum-address'
      ].toLowerCase();
      ...
      let accountTwoApprovedBuyer = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-approved-buyers', accountTwo);
      expect(accountTwoApprovedBuyer.data.attributes['mapping-boolean-value']).to.equal(
        true,
        'the mapping-boolean-value is correct',
      );

      let accountTwoCustomBuyerLimit = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-custom-buyer-limits', accountTwo);
      expect(accountTwoCustomBuyerLimit.data.attributes['mapping-number-value']).to.equal(
        '10',
        'the mapping-number-value is correct',
      );
    });
  });
  ...
  describe('ethereum-indexer event triggers', function() {
    ...
    async function setup() {
      ...
      factory.addResource('data-sources', contractName).withAttributes({
        'source-type': '@cardstack/ethereum',
        params: {
          branches: {
            master: { jsonRpcUrl: 'ws://localhost:7545' },
    ...
    it('can update contract document from event content trigger', async function() {
      let contract = await env.lookup('hub:searchers').get(env.session, 'master', 'sample-tokens', token.address);
      expect(contract.data.attributes['sample-token-minting-finished']).to.equal(
        false,
        'the minting-finished field is correct',
      );
      ...
      let event = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-mint-finished-events', eventId);
      expect(event.data.attributes['block-number']).to.equal(events[0].blockNumber, 'the block number is correct');
      expect(event.data.attributes['transaction-id']).to.equal(
  ...
  describe('ethereum-indexer for past events', function() {
    ...
    async function setup() {
      ...
      let dataSource = factory.addResource('data-sources', contractName).withAttributes({
        'source-type': '@cardstack/ethereum',
        params: {
          branches: {
            master: { jsonRpcUrl: 'ws://localhost:7545' },
    ...
    it('can capture events documents from events that were fired before hub started', async function() {
      ...
      let transferEvent = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-transfer-events', transferEventId);
      toLowercase(transferEvent, 'data.attributes.transfer-event-from');
      toLowercase(transferEvent, 'data.attributes.transfer-event-to');
      expect(transferEvent.data.attributes['block-number']).to.equal(
        mintEvents[1].blockNumber,
        'the block number is correct',
      );
      expect(transferEvent.data.attributes['transaction-id']).to.equal(
        mintEvents[1].transactionHash,
        'the transaction id is correct',
      );
      expect(transferEvent.data.attributes['event-name']).to.equal('Transfer', 'the event name is correct');
      expect(transferEvent.data.attributes['transfer-event-from']).to.equal(
        NULL_ADDRESS,
        'the transaction event from address is correct',
      );
      expect(transferEvent.data.attributes['transfer-event-to']).to.equal(
        accountOne,
        'the transaction event to address is correct',
      );
      expect(transferEvent.data.attributes['transfer-event-value']).to.equal(
        '100',
        'the transaction event value is correct',
      );
    });
    ...
    it('indexes past events that occur at a block height heigher than the last time it has indexed', async function() {
      ...
      pgclient.on('add', ({ type }) => {
        if (!['sample-tokens', 'sample-token-balance-ofs'].includes(type)) {
          return;
        }
        addCount++;
      });
    ...
    it('skips indexing when contract state indicates that the contract is not prepared to be indexed', async function() {
      ...
      expect(accountOneLedgerEntry.data.attributes['mapping-number-value']).to.equal(
        '80',
        'the token balance is correct',
      );
      expect(accountTwoLedgerEntry.data.attributes['mapping-number-value']).to.equal(
  ...
  describe('ethereum-indexer can patch schema', function() {
    ...
    async function setup() {
      ...
      (contract = dataSource.data.attributes.params.contract),
        (env = await createDefaultEnvironment(
          `${__dirname}/../../../tests/ethereum-computed-fields`,
          factory.getModels(),
        ));
    ...
    it('can patch schema with computed-fields', async function() {
      ...
      let accountOneBalance = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountOne);
      let accountThreeBalance = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountThree);

      expect(accountOneBalance.data.attributes['utility-payment-amount']).to.equal(
        50,
        'utility-payment-amount computed field is correct',
      );
      expect(accountThreeBalance.data.attributes['utility-payment-amount']).to.equal(
        20,
        'utility-payment-amount computed field is correct',
      );
    });
  ...
  describe('ethereum-indexer for a large amount of past events', function() {
    ...
    it('can index a large amount of past events on the contract', async function() {
      let accountOneLedgerEntry = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountOne.toLowerCase());
      let accountTwoLedgerEntry = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountTwo.toLowerCase());
      let accountThreeLedgerEntry = await env
        .lookup('hub:searchers')
        .get(env.session, 'master', 'sample-token-balance-ofs', accountThree.toLowerCase());
      let tokenRecord = await env.lookup('hub:searchers').get(env.session, 'master', 'sample-tokens', token.address);
      let events = await env
        .lookup('hub:searchers')
        .search(env.session, 'master', { page: { size: 5000 }, filter: { type: 'sample-token-white-list-events' } });
      ...
      expect(accountThreeLedgerEntry.data.attributes['mapping-number-value']).to.equal(
        '37',
        'the token balance is correct',
      );
      expect(events.data.length).to.equal(
        addresses.length,
        'the correct number of whitelist event documents were created',
      );

      for (let address of addresses) {
        let whitelistEntry = await env
          .lookup('hub:searchers')
          .get(env.session, 'master', 'sample-token-approved-buyers', address.toLowerCase());
