...
module.exports = declareInjections(
  ...
  class EthereumEventIndexer {
    ...
    async index(contractName, blockHeights, history) {
      this._indexingPromise = Promise.resolve(this._indexingPromise).then(() =>
        this._processRecords({ contractName, blockHeights, history }),
      );
    ...
    async _processRecords({ contractName, blockHeights, history }) {
      log.debug(
        `processing records for ethereum with last indexed blockheights ${JSON.stringify(
          blockHeights,
      ...
      for (let branch of Object.keys(contractDefinition.addresses)) {
        ...
        await this._indexRecord(
          batch,
          attachMeta(await this.ethereumClient.getContractInfo({ branch, contract: contractName }), {
            blockheight,
            branch,
            contractName,
          }),
      ...
      if (!history || !history.length) {
        log.info(
          `Retreving full history for contract ${contractName} address: ${JSON.stringify(
            contractDefinition.addresses,
          )} since blockheight ${JSON.stringify(blockHeights)}`,
        );
      ...
      for (let { branch, event, identifiers } of history) {
        ...
        if (eventModel) {
          ...
          if (!contractDefinition.eventContentTriggers[eventModel.attributes['event-name']].length) {
            ...
            if (!contract) {
              log.error(
                `Cannot find contract ${pluralize(
                  contractName,
                )}/${contractAddress} from index when trying to associate event to contract ${JSON.stringify(
                  eventModel,
                )}`,
              );
              continue;
        ...
        for (let { id, type, isContractType } of identifiers) {
          if (!branch || !type || !id || isContractType) {
            continue;
          }

          let contractAddress = contractDefinition.addresses[branch];
          let blockheight = await this.ethereumClient.getBlockHeight(branch);
          let { data, methodName } = await this.ethereumClient.getContractInfoForIdentifier({
            id,
            type,
            branch,
            contractName,
          });
          let methodAbiEntry = contractDefinition['abi'].find(
            item => item.type === 'function' && item.constant && item.name === methodName,
          );
