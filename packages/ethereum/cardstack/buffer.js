...
class EthereumBuffer {
  ...
  async _processHints({ contractName, blockHeights, hints }) {
    ...
    this._bufferedRecordIndexingPromise = Promise.resolve(this._bufferedRecordIndexingPromise)
      .then(() => {
        log.debug(`queuing index jobs of buffered ethereum models with last indexed blockheights ${JSON.stringify(blockHeights)}, hints: ${JSON.stringify(hints)}`);
        return this.indexer.update({
          forceRefresh: true,
