...
function testPackage(package) {
  return new Promise((res, reject) => {
    ...
    let proc = spawn(process.execPath, [truffleBin, 'test'], { stdio: ['ignore', process.stdout, process.stderr], cwd: package });
...
async function run() {
  ...
  for (let package of packages) {
    process.stdout.write(`Starting truffle test suite for ${path.basename(package)}\n`);
