...
function testPackage(pkg) {
  return new Promise((res, reject) => {
    ...
    let proc = spawn(process.execPath, [emberBin, 'test'], {
...
async function run(cardsDirectory) {
  let packages = glob
    .sync(path.join(process.cwd(), cardsDirectory, '*', 'ember-cli-build.js'))
...
run((process.argv.length > 2 && process.argv[2]) || '.').then(
  () => process.exit(0),
  err => {
