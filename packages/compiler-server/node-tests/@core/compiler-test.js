...
const PERSON_CARD = {
    ...
    files: {
      ...
      export default class Person {
        ...
        'embedded.js': templates_1.templateOnlyComponentTemplate('<div class="person-embedded"><@fields.name/> was born on <@fields.birthdate/></div>'),
...
describe('Compiler', function () {
    ...
    it('string card', async function () {
        ...
        expect((_a = compiled.adoptsFrom) === null || _a === void 0 ? void 0 : _a.url).to.equal(compiler_1.baseCardURL);
    ...
    it('CompiledCard embedded view', async function () {
        builder.addRawCard(PERSON_CARD);
        let compiled = await builder.getCompiledCard(PERSON_CARD.url);
        expect(builder.definedModules.get(compiled.embedded.moduleName)).to.containsSource('{{@model.name}} was born on <HttpsCardstackComBaseDateField @model={{@model.birthdate}} data-test-field-name=\\"birthdate\\" />');
        expect(builder.definedModules.get('https://cardstack.local/cards/person/embedded.css'), 'Styles are defined').to.containsSource(PERSON_CARD.files['embedded.css']);
    });
    it('CompiledCard edit view', async function () {
        builder.addRawCard(PERSON_CARD);
        let compiled = await builder.getCompiledCard(PERSON_CARD.url);
        expect(compiled.edit.usedFields).to.deep.equal(['name', 'birthdate']);
        expect(builder.definedModules.get(compiled.edit.moduleName), 'Edit template is rendered for text').to.containsSource('<HttpsCardstackComBaseStringField @model={{@model.name}} data-test-field-name=\\"name\\" @set={{@set.setters.name}} />');
        expect(builder.definedModules.get(compiled.edit.moduleName), 'Edit template is rendered for date').to.containsSource('<HttpsCardstackComBaseDateField @model={{@model.birthdate}}  data-test-field-name=\\"birthdate\\" @set={{@set.setters.birthdate}} />');
    });
    it('nested cards', async function () {
        ...
        builder.addRawCard({
          ...
          export default class Post {
            ...
            author;
                ...
                'embedded.js': templates_1.templateOnlyComponentTemplate(`<article><h1><@fields.title /></h1><p><@fields.author.name /></p><p><@fields.author.birthdate /></p></article>`),
                'isolated.js': templates_1.templateOnlyComponentTemplate(`<SomeRandoComponent @attr={{@model.author}} />`),
            },
        });
        ...
        expect(builder.definedModules.get(compiled.embedded.moduleName)).to.containsSource(`<article><h1>{{@model.title}}</h1><p>{{@model.author.name}}</p><p><HttpsCardstackComBaseDateField @model={{@model.author.birthdate}} data-test-field-name=\\"birthdate\\"  /></p></article>`);
    ...
    it('deeply nested cards', async function () {
      ...
      `,
                'isolated.js': templates_1.templateOnlyComponentTemplate(`<h1><@fields.title /></h1><h2><@fields.createdAt /></h2><p>{{@model.body}}</p>`),
      ...
      `,
                'isolated.js': templates_1.templateOnlyComponentTemplate('{{#each @fields.posts as |Post|}}<Post />{{/each}}'),
                'embedded.js': templates_1.templateOnlyComponentTemplate('<ul>{{#each @fields.posts as |Post|}}<li><Post.title /></li>{{/each}}</ul>'),
            },
        });
        ...
        expect(builder.definedModules.get(compiled.isolated.moduleName), 'Isolated template includes PostField component').to.containsSource(`{{#each @model.posts as |Post|}}<HttpCardstackLocalCardsPostField @model={{Post}} data-test-field-name=\\"posts\\" />{{/each}}`);
        expect(compiled.embedded.usedFields).to.deep.equal(['posts.title']);
        expect(builder.definedModules.get(compiled.embedded.moduleName), 'Embedded template inlines post title').to.containsSource(`<ul>{{#each @model.posts as |Post|}}<li>{{Post.title}}</li>{{/each}}</ul>`);
    });
    describe('@fields iterating', function () {
        let postCard = {
          ...
          export default class Post {
            ...
            title;
                ...
                'embedded.js': templates_1.templateOnlyComponentTemplate(`<article>{{#each-in @fields as |name|}}<label>{{name}}</label>{{/each-in}}</article>`),
            },
        };
        ...
        it('iterators of fields and inlines templates', async function () {
            let compiled = await builder.getCompiledCard('https://cardstack.local/cards/post');
            expect(builder.definedModules.get(compiled.embedded.moduleName)).to.containsSource('<article><label>{{\\"title\\"}}</label></article>');
        });
        it('recompiled parent field iterator', async function () {
          ...
          export default @adopts(Post) class TimelyPost {
            ...
            expect(builder.definedModules.get(timelyCompiled.embedded.moduleName)).to.containsSource('<article><label>{{\\"title\\"}}</label><label>{{\\"createdAt\\"}}</label></article>');
            expect(builder.definedModules.get(fancyCompiled.embedded.moduleName)).to.containsSource('<article><label>{{\\"title\\"}}</label><label>{{\\"body\\"}}</label></article>');
        });
    });
});
