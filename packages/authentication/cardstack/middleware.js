...
module.exports = declareInjections(
  ...
  class Authentication {
    ...
    async _tokenToSession(token, ctxt) {
      ...
      try {
        ...
        if (/unable to authenticate data|invalid key length|Not a valid signed message/.test(err.message)) {
          log.warn(
            `Ignoring invalid token for ${ctxt.request.method} ${ctxt.request.path} (${ctxt.header['authorization']})`
          );
        } else {
          throw err;
        }
        ...
        compose([
          async ctxt => {
            ...
            await withJsonErrorHandling(ctxt, async () => {
              ...
              if (!user) {
                throw new Error(`cant find user type ${session.type} id ${session.id}`);
              }
    ...
    async _sessionMeta(ctxt) {
      ...
      try {
        config = await this.searcher.get(
      ...
      if (
        config &&
        config.data.attributes['plugin-config'] &&
        config.data.attributes['plugin-config']['allow-x-forwarded-for']
      ) {
    ...
    async _applyReadAuthorization(session, user) {
      ...
      let authorizedUser = await documentContext.applyReadAuthorization({ session });
      if (!authorizedUser) {
