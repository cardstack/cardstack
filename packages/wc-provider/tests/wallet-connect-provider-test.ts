/**
 * Modified by Cardstack Foundation:
 * - Makes the provider work with websockets instead of http
 *
...
import WalletConnectProvider, { ICardstackWalletConnectProviderOptions } from '..';

const CHAIN_ID = 123;
const PORT = 8546;
const MOCK_KOVAN_PORT = 8547;
const RPC_URL = `http://localhost:${PORT}`;
const RPC_WSS_URL = `ws://localhost:${PORT}`;
const MOCK_KOVAN_WSS_URL = `ws://localhost:${MOCK_KOVAN_PORT}`;
const ACCOUNTS = {
  a: {
...
const TEST_PROVIDER_OPTS: ICardstackWalletConnectProviderOptions = {
  chainId: CHAIN_ID,
  qrcode: false,
...
describe('WalletConnectProvider', function () {
  ...
  before(async () => {
    ...
    mockKovanTestNetwork = await TestNetwork.init({
      chainId: 42,
      port: MOCK_KOVAN_PORT,
      genesisAccounts: [ACCOUNTS.c],
    });
  ...
  after(async () => {
    ...
    await mockKovanTestNetwork.close();
    // disconnect provider
    await Promise.all([
  ...
  it('chainChanged', async () => {
    ...
    await Promise.all([
      new Promise<void>(async (resolve, reject) => {
        try {
          await walletClient.changeChain(42, MOCK_KOVAN_WSS_URL);
          resolve();
        } catch (e) {
          reject(e);
        }
    ...
    await Promise.all([
      new Promise<void>(async (resolve, reject) => {
        try {
          await walletClient.changeChain(CHAIN_ID, RPC_WSS_URL);
          resolve();
        } catch (e) {
          reject(e);
        }
