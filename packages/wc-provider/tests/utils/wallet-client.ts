...
export class WalletClient {
  ...
  private setChain(chainId: number, rpcUrl: string) {
    ...
    if (this.rpcUrl !== rpcUrl) {
      this.rpcUrl = rpcUrl;
      this.signer = this.signer.connect(new ethers.providers.WebSocketProvider(this.rpcUrl));
    }
  }
  ...
  private getWallet(privateKey?: string) {
    ...
    return wallet.connect(new ethers.providers.WebSocketProvider(this.rpcUrl));
  }
    ...
    this.provider.connector.on('display_uri', (error: any, payload: any) => {
      ...
      this.client.on('call_request', async (error, payload) => {
        ...
        try {
          ...
          const message = (e as Error).message || (e as Error).toString();
