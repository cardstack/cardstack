...
module('Integration | @cardstack/data service query()', function(hooks) {
  setupTest(hooks);
  ...
  module('no defaultIncludes', function(hooks) {
    ...
    test('it can load relationships for a query in a particular format', async function(assert) {
      ...
      assert.ok(
        error.message.match(
          /You looked up the 'location' relationship on a 'puppy' with id vanGogh but some of the associated records were not loaded/
        )
      );
      ...
      assert.ok(
        error.message.match(
          /You looked up the 'dog' relationship on a 'doggy-article' with id swim but some of the associated records were not loaded/
        )
      );
    ...
    test('it can load relationships for a queryCard in a particular format', async function(assert) {
      let dataService = this.owner.lookup('service:cardstackData');
      let homepage = await run(() =>
        dataService.queryCard('isolated', { filter: { title: 'Homepage' }, type: 'page' })
      );
  ...
  module('with defaultIncludes', function(hooks) {
    ...
    test('it loads the defaultIncludes when a format is requested that is not in the fieldset', async function(assert) {
      ...
      assert.ok(
        error.message.match(
          /You looked up the 'author' relationship on a 'puppy-article' with id bones but some of the associated records were not loaded/
        )
      );
  ...
  module('defaultIncludes vs fieldset clash', function(hooks) {
    ...
    test("it does not load a relationship that is not in a request format's fieldset but is defaultIncluded", async function(assert) {
      ...
      assert.ok(
        error.message.match(
          /You looked up the 'relatedArticle' relationship on a 'puppy-article' with id bones but some of the associated records were not loaded/
        )
      );
