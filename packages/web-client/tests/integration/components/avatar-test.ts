...
const LOADING = '[data-test-avatar-loading]';

const PLACEHOLDER_ATTRIBUTE = 'data-test-avatar-placeholder';
...
module('Integration | Component | avatar', function (hooks) {
  ...
  test('it can render an image and its alt', async function (assert) {
    this.setProperties({
      src: imageDataUri,
      alt: 'An alt',
    });
    ...
    assert.dom(IMAGE).hasAttribute('alt', 'An alt');
  });

  test('it can render an image in a loading state', async function (assert) {
    this.setProperties({
      src: imageDataUri,
      loading: true,
    });
  ...
  test('it allows a rounded image', async function (assert) {
    this.setProperties({
      rounded: true,
    });
    await render(hbs`
    ...
    assert.dom(AVATAR).hasClass(/--rounded/);
  });

  test('it can render a placeholder and allows customization', async function (assert) {
    await render(hbs`
      <Avatar
    ...
    // default
    assert.dom(`[${PLACEHOLDER_ATTRIBUTE}="user"]`).isVisible();

    this.setProperties({
      placeholderIcon: 'trash',
    });

    assert.dom(`[${PLACEHOLDER_ATTRIBUTE}="trash"]`).isVisible();
  });
});
