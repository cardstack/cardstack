...
export default class ImageEditorComponent extends Component<ImageEditorComponentArguments> {
  ...
  @action initializeCropper(image: HTMLImageElement) {
    try {
      ...
      console.error('failed to initialize image cropper');
      this.onCrash(e);
    }
  }
  ...
  updatePreview() {
    try {
      ...
      console.error('failed to update preview while editing image');
      this.onCrash(e);
    }
  }
  ...
  @action async save() {
    ...
    try {
      ...
      console.error('failed to save image as file while editing image');
      this.onCrash(e);
      return;
    }
