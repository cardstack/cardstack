...
class CardPaySafeBalanceCardComponent extends Component<CardPaySafeBalanceCardComponentArgs> {
  ...
  constructor(owner: unknown, args: CardPaySafeBalanceCardComponentArgs) {
    ...
    if (!this.safeAddress) {
      throw new Error(
        `CardPay::SafeBalanceCard requires the ${this.safeAddressKey} referenced by safeBalanceCardKey in the workflow session`
      );
    }
  ...
  get safeAddress() {
    return this.args.workflowSession.getValue<string>(this.safeAddressKey!);
  }
