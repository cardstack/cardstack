...
module('Acceptance | Workflow', function(hooks) {
  ...
  function find() {
    throw new Error(
      'these tests need to be refactored away from "find", but they are skipped right now so I\'m leaving them alone',
    );
  }
  ...
  hooks.beforeEach(function(assert) {
    ...
    assert.unhandledAlert = () => {
      this.equal(
        this.element.querySelectorAll('[data-test-alert-notification]').length,
        1,
        'The unhandled alert notification appears',
      );
    };
    assert.groupCount = (groupName, value, assertionText) => {
      assert.trimmedText(
        assert,
        this.element.querySelector(`[data-test-group-counter="${groupName}"]`),
        value,
        assertionText,
      );
    };
  ...
  skip('List threads that match the clicked tag', async function(assert) {
    ...
    assert.equal(
      find('.cardstack-workflow-label-with-count-wrapper.active:contains(License Request)').length,
      1,
      'The selected group is marked as active',
    );
    assert.equal(find('[data-test-thread-list-card]:contains(This is going to be tough, my friend.)').length, 1);
    assert.equal(
      find("[data-test-thread-list-card]:contains(License request for Caspian's Sycamore, please?)").length,
      1,
    );
  ...
  skip('List threads that match the Today date range', async function(assert) {
    ...
    assert.equal(
      find('.cardstack-workflow-label-with-count-wrapper.active:contains(Today)').length,
      1,
      'The selected group is marked as active',
    );
  });
  ...
  skip('Switch between thread lists and an individual thread view', async function(assert) {
    ...
    assert.equal(
      find('[data-test-message-card]:contains("Could we add yet more guitars to this Caspian song?")').length,
      2,
    );
    ...
    await click('[data-test-thread-list-card]:first');
    assert.equal(
      find('[data-test-message-card]:contains("Could we change our previous cover of Pearl Jam\'s Daughter?")').length,
      2,
    );
  ...
  skip('Take action on a cue card', async function(assert) {
    ...
    assert.groupCount(
      'Need Response::License Request',
      1,
      'Unhandled group count is decremented after approving a message',
    );
