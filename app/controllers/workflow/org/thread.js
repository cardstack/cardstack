...
export default class WorkflowOrgThreadController extends Controller {
  ...
  get userTasks() {
    if (!this.activeTasks || !this.model.user) {
      return null;
    }
    return this.activeTasks.filter(
      (el) => el.assigned_to === this.model.user.id
    );
  }
  ...
  get assignedTasks() {
    ...
    return this.activeTasks.filter(
      (el) =>
        (el.assigned_by === this.model.user.id ||
          el.assigned_by === 'system') &&
        el.assigned_to !== this.model.user.id
    );
  }
}
